...
// ---------------------------------------------------------------------------
//  Arp button panel - two toggles + two radio groups of four
// ---------------------------------------------------------------------------

// Draws one button: lit uses the active colour, unlit is dark.
// Draws one PB86-A1-style button. The cap stays constant; the LED shows state.
// PB86-A1-style button: shallow LED shelf on top, deep lower face for the legend.
void drawArpButton(int x, int y, int w, const char *label, bool lit) {
  int h = BTN_H;

  // Cap body: black fill + 3px grey border
  tft.fillRoundRect(x, y, w, h, BTN_R, COL_CAP_FACE);
  tft.drawRoundRect(x,     y,     w,     h,     BTN_R,     COL_CAP_BORDER);
  tft.drawRoundRect(x + 1, y + 1, w - 2, h - 2, BTN_R - 1, COL_CAP_BORDER);
  tft.drawRoundRect(x + 2, y + 2, w - 4, h - 4, BTN_R - 2, COL_CAP_BORDER);

  // Thin line marking the LED shelf / face split
  int faceTop = y + LED_SHELF;
  tft.drawFastHLine(x + 4, faceTop, w - 8, COL_CAP_HI);

  // LED, centred on the top shelf
  int lx = x + w / 2;
  int ly = y + LED_SHELF / 2;
  if (lit) {
    tft.fillSmoothCircle(lx, ly, 4, COL_LED_ON, COL_CAP_FACE);
    tft.drawPixel(lx - 1, ly - 1, COL_LED_HALO);
    tft.drawPixel(lx,     ly - 1, COL_LED_HALO);
  } else {
    tft.fillSmoothCircle(lx, ly, 4, COL_LED_OFF, COL_CAP_FACE);
    tft.drawSmoothCircle(lx, ly, 4, COL_LED_OFF_RIM, COL_CAP_FACE);
  }

  // Legend, centred in the deep lower face
  tft.setFreeFont(NULL);
  tft.setTextFont(2);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TFT_WHITE, COL_CAP_FACE);   // white on black
  tft.drawString(label, lx, faceTop + (h - LED_SHELF) / 2 - 1);
}

void drawArpHeader(const char *label, int x, int y, int w) {
  tft.setFreeFont(NULL);
  tft.setTextFont(2);                         // was font 1
...
void drawArpPanel() {
  ...
  // Row 1 - toggles
  drawArpButton(lx,        12, 106, "ARP ON", panelData[PARP_ONOFF] != 0);
  drawArpButton(lx + 114,  12, 106, "LATCH",  panelData[PARP_LATCH] != 0);

  // Row 2 - octave range (value 1..4)
  const char *rng[4] = { "1", "2", "3", "4" };

  for (int k = 0; k < 4; k++)
    drawArpButton(lx + k * 56, 88, 52, rng[k], panelData[PARP_RANGE] == k + 1);

  // Row 3 - mode (value 1..4)
  const char *mode[4] = { "UP", "DN", "UP/DN", "RND" };

  for (int k = 0; k < 4; k++)
    drawArpButton(lx + k * 56, 164, 52, mode[k], panelData[PARP_MODE] == k + 1);
}
...
void myLEDupdate(byte channel, byte control, int value) {
  switch (control) {
    ...
    case CCarpStartStop:
      panelData[PARP_ONOFF] = value;
      drawArpPanel();
      break;

    case CCarpLatch:
      panelData[PARP_LATCH] = value;
      drawArpPanel();
      break;

    case CCarpRange:
      panelData[PARP_RANGE] = value;   // expects 0..3
      drawArpPanel();
      break;

    case CCarpMode:
      panelData[PARP_MODE] = value;    // expects 0..3
      drawArpPanel();
      break;
  }
}
