...
Knob knobs[NUM_KNOBS] = {
  ...
  EMPTY_SLOT,
  EMPTY_SLOT,
  KNOB(CCglideTime, P_glideTime, "TIME"),

  EMPTY_SLOT,
  EMPTY_SLOT,
  EMPTY_SLOT,
  CENTRE(CCdualDetune, P_dualDetune, "DUAL"),

  EMPTY_SLOT,
  EMPTY_SLOT,
  EMPTY_SLOT,
  KNOB(CCunisonDetune, P_unisonDetune, "UNISON"),
};
...
// One LED, drawn at (lx,ly). Bright when on, dark when off.
void drawLED(int lx, int ly, uint16_t onCol, uint16_t offCol, bool on) {
  if (on) {
    tft.fillSmoothCircle(lx, ly, 4, onCol, COL_CAP_FACE);
    tft.drawPixel(lx - 1, ly - 1, COL_LED_HALO);
  } else {
    tft.fillSmoothCircle(lx, ly, 4, offCol, COL_CAP_FACE);
  }
}

void drawCap(int x, int y, int w, const char *label) {
  tft.fillRoundRect(x, y, w, BTN_H, BTN_R, COL_CAP_FACE);        // black fill
  tft.drawRoundRect(x, y, w, BTN_H, BTN_R, COL_CAP_BORDER);      // grey border
  tft.drawRoundRect(x + 2, y + 2, w - 4, BTN_H - 4, BTN_R - 2, COL_CAP_BORDER);  // 3px

  // Thin line marking the LED shelf / face split
  int faceTop = y + LED_SHELF;
  tft.drawFastHLine(x + 3, faceTop, w - 6, COL_CAP_HI);

  tft.setFreeFont(NULL);
  tft.setTextFont(2);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TFT_WHITE, COL_CAP_FACE);      // white legend on black
  tft.drawString(label, x + w / 2, faceTop + (BTN_H - LED_SHELF) / 2 - 1);
}

// Single-LED button (radio member or plain toggle).
void drawBtn1(int x, int y, int w, const char *label, bool lit) {
  drawCap(x, y, w, label);
  drawLED(x + w / 2, y + LED_SHELF / 2, COL_RED_ON, COL_RED_OFF, lit);
}

void drawHeader(const char *label, int x, int y, int w) {
  tft.setFreeFont(NULL);
  tft.setTextFont(2);
  tft.setTextColor(COL_HDR, TFT_BLACK);
  tft.setTextDatum(TC_DATUM);
  tft.drawString(label, x + w / 2, y);
}

// NotePriority: 0 = green only, 1 = red only, 2 = both.
void drawBtn2(int x, int y, int w, const char *label, int state) {
  drawCap(x, y, w, label);
  int cy = y + LED_SHELF / 2;
  drawLED(x + w / 2 - 9, cy, COL_RED_ON, COL_RED_OFF, state == 1 || state == 2);  // RED on 1,2
  drawLED(x + w / 2 + 9, cy, COL_GRN_ON, COL_GRN_OFF, state == 0 || state == 2);  // GRN on 0,2
}

// Dual-LED, PlayMode order: 0 = red, 1 = both, 2 = green.
void drawBtn2play(int x, int y, int w, const char *label, int state) {
  drawCap(x, y, w, label);
  int cy = y + LED_SHELF / 2;
  // wipe the LED shelf so no previous-state pixels survive
  tft.fillRect(x + 4, y + 3, w - 8, LED_SHELF - 4, COL_CAP_FACE);
  drawLED(x + w / 2 - 9, cy, COL_RED_ON, COL_RED_OFF, state == 0 || state == 1);
  drawLED(x + w / 2 + 9, cy, COL_GRN_ON, COL_GRN_OFF, state == 1 || state == 2);
}

void drawGlideButton(int x, int y, int w) {
  drawCap(x, y, w, "GLIDE");
  int cy = y + LED_SHELF / 2;

  bool lowerGlide = lowerData[P_glideSW] != 0;   // CHECK index name
  bool upperGlide = upperData[P_glideSW] != 0;

  bool red, grn;
  if (playMode == 0) {
    bool on = lowerGlide || upperGlide;   // whole: linked, either reflects both
    red = on;
    grn = on;
  } else {
    red = lowerGlide;   // dual/split: per layer
    grn = upperGlide;
  }

  drawLED(x + w / 2 - 9, cy, COL_RED_ON, COL_RED_OFF, red);
  drawLED(x + w / 2 + 9, cy, COL_GRN_ON, COL_GRN_OFF, grn);
}

void drawVoicePanel() {
  const int lx = 8;

  // Row 1 - NotePriority + Glide, dual-LED
  drawBtn2(lx, 12, 106, "PRIORITY", panelData[P_NotePriority]);
  drawGlideButton(lx + 114, 12, 106);

  // Row 2 - Lower/Upper radio pair + Hold toggle + Play dual-LED (whole/dual/split)
  drawBtn1(lx,       88, 52, "LOWER", lowerSW != 0);
  drawBtn1(lx + 56,  88, 52, "UPPER", upperSW != 0);
  drawBtn1(lx + 112, 88, 52, "HOLD", panelData[P_holdSW] != 0);
  drawBtn2play(lx + 168, 88, 52, "PLAY", playMode);  // global int, not panelData

  // Row 3 - keyboard mode radio: Poly1/Poly2/Unison/Mono (1..4)
  const char *km[4] = { "POLY1", "POLY2", "UNIS", "MONO" };
  for (int k = 0; k < 4; k++)
    drawBtn1(lx + k * 56, 164, 52, km[k], panelData[P_keyboardMode] == k);
}
...
void myLEDupdate(byte channel, byte control, int value) {
  switch (control) {
    ...
    case CClowerSW:     // panel Lower button - selects the lower layer
      lowerSW = 1;
      upperSW = 0;
      drawVoicePanel();
      break;

    case CCupperSW:     // panel Upper button - selects the upper layer
      upperSW = 1;
      lowerSW = 0;
      drawVoicePanel();
      break;
    ...
    case CCglideSW:
      if (playMode == 0) {
        lowerData[P_glideSW] = value;    // whole mode: write both layers together
        upperData[P_glideSW] = value;
      } else if (lowerSW) {
        lowerData[P_glideSW] = value;    // dual/split: write the selected layer
      } else if (upperSW) {
        upperData[P_glideSW] = value;
      }
      drawVoicePanel();
    ...
    case CCchordHoldSW:  // Hold toggle - CHECK this is the right CC
      panelData[P_holdSW] = value;
      drawVoicePanel();
      break;

    case CCplayMode:  // whole/dual/split - writes the global
      playMode = value;
      drawVoicePanel();
      break;

    case CCkeyboardMode:  // Poly1/Poly2/Unison/Mono radio
