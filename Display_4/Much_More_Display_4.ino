...
Knob knobs[NUM_KNOBS] = {
  KNOB(CCfilterLevel1, P_filterLevel1, "LEV 1"),
  KNOB(CCfilterCutoff, P_filterCutoff, "FREQ"),
  KNOB(CCfilterRes, P_filterRes, "RES"),
  KNOB(CCfilterEGlevel, P_filterEGlevel, "ENV"),


  KNOB(CCfilterLevel2, P_filterLevel1, "LEV 2"),
  KNOB(CCkeyTrack, P_keytrack, "TRK"),
  KNOB(CCfilterLFO, P_filterLFO, "LFO"),
  KNOB(CCnoiseLevel, P_noiseLevel, "NOISE"),
...
void renderCurrentPatchPage() {
  ...
  tft.fillRoundRect(10, 180, 320-20, 60, 5, getActiveColor());  // Background box for range
...
void myLEDupdate(byte channel, byte control, int value) {
  switch (control) {
    ...
    case CCfilterPoleSW:
      panelData[P_filterPoleSW] = value;
      switch (panelData[P_filterPoleSW]) {
        case 1:
          str_ptr = reinterpret_cast<const char *>(pgm_read_ptr(&(filter01[panelData[P_filterType]])));
          break;
