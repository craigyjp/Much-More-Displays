...
Knob knobs[NUM_KNOBS] = {
  EMPTY_SLOT,
  KNOB(CCpitchAttack, P_pitchAttack, "P ATT"),
  KNOB(CCpitchDecay, P_pitchDecay, "P DEC"),
  EMPTY_SLOT,


  EMPTY_SLOT,
  KNOB(CCpitchSustain, P_pitchSustain, "P SUS"),
  KNOB(CCpitchRelease, P_pitchRelease, "P REL"),
  EMPTY_SLOT,
...
void drawPitchEnv() {
  bool adr = panelData[P_env1_adsr] != 0;
  drawEnvADSR(panelData[P_pitchAttack],  panelData[P_pitchDecay],
              panelData[P_pitchSustain], panelData[P_pitchRelease],
              PITCH_BOX_X, PITCH_BOX_Y, PITCH_BOX_W, PITCH_BOX_H,
...
void myControlChange(byte channel, byte control, int value) {
  for (int i = 0; i < NUM_KNOBS; i++) {
    ...
    if (control == CCpitchAttack  || control == CCpitchDecay ||
    control == CCpitchSustain || control == CCpitchRelease) {
    drawPitchEnv();
  }
}
